...
class HealthFactory {
  ...
  /// Check access to GoogleFit/Apple HealthKit
  Future<bool> checkAuthorization(List<HealthDataType> types) async {
    ...
    await _channel.invokeMethod('checkAuthorization', {'types': keys});
    return isAuthorized;
  }

  /// Logout to GoogleFit Account
  Future<bool> logoutGoogleAccount(List<HealthDataType> types) async {
    /// If BMI is requested, then also ask for weight and height
    if (types.contains(HealthDataType.BODY_MASS_INDEX)) {
      if (!types.contains(HealthDataType.WEIGHT)) {
        types.add(HealthDataType.WEIGHT);
    ...
    List<String> keys = types.map((e) => _enumToString(e)).toList();
    final bool isLogout = await _channel.invokeMethod('logoutGoogleAccount', {"types": keys});
    return isLogout;
  }
